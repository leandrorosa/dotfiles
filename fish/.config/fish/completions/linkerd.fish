...
function __linkerd_perform_completion
    __linkerd_debug "Starting __linkerd_perform_completion"
    ...
    set -l requestComp "LINKERD_ACTIVE_HELP=0 $args[1] __complete $args[2..-1] $lastArg"
...
# - Obtain the completions and store them in the global __linkerd_comp_results
# - Return false if file completion should be performed
function __linkerd_prepare_completions
    __linkerd_debug ""
    __linkerd_debug "========= starting completion logic =========="

    # Start fresh
    set --erase __linkerd_comp_results

    set -l results (__linkerd_perform_completion)
    ...
    if test $filefilter -eq 1; or test $dirfilter -eq 1
        __linkerd_debug "File extension filtering or directory filtering not supported"
    ...
    if test $nospace -ne 0; or test $nofiles -eq 0
        ...
        __linkerd_debug "prefix: $prefix"

        set -l completions (string match -r -- "^$prefix.*" $__linkerd_comp_results)
        set --global __linkerd_comp_results $completions
        __linkerd_debug "Filtered completions are: $__linkerd_comp_results"

        # Important not to quote the variable for count to work
        set -l numComps (count $__linkerd_comp_results)
        ...
        if test $numComps -eq 1; and test $nospace -ne 0
            ...
            set -l split (string split --max 1 \t $__linkerd_comp_results[1])
            ...
            if not string match -r -q "[@=/:.,]" -- "$lastChar"
                ...
                set --global __linkerd_comp_results $split[1] $split[1].
                __linkerd_debug "Completions are now: $__linkerd_comp_results"
            end
        end
...
if type -q "linkerd"
    ...
    complete --do-complete "linkerd " > /dev/null 2>&1
end

# Remove any pre-existing completions for the program since we will be handling all of them.
complete -c linkerd -e

# The call to __linkerd_prepare_completions will setup __linkerd_comp_results
