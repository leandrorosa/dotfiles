...
function __devspace_perform_completion
    __devspace_debug "Starting __devspace_perform_completion"
...
# - Obtain the completions and store them in the global __devspace_comp_results
# - Return false if file completion should be performed
function __devspace_prepare_completions
    __devspace_debug ""
    __devspace_debug "========= starting completion logic =========="

    # Start fresh
    set --erase __devspace_comp_results

    set -l results (__devspace_perform_completion)
    ...
    if test $filefilter -eq 1; or test $dirfilter -eq 1
        __devspace_debug "File extension filtering or directory filtering not supported"
    ...
    if test $nospace -ne 0; or test $nofiles -eq 0
        ...
        __devspace_debug "prefix: $prefix"

        set -l completions (string match -r -- "^$prefix.*" $__devspace_comp_results)
        set --global __devspace_comp_results $completions
        __devspace_debug "Filtered completions are: $__devspace_comp_results"

        # Important not to quote the variable for count to work
        set -l numComps (count $__devspace_comp_results)
        ...
        if test $numComps -eq 1; and test $nospace -ne 0
            ...
            set -l split (string split --max 1 \t $__devspace_comp_results[1])
            ...
            if not string match -r -q "[@=/:.,]" -- "$lastChar"
                ...
                set --global __devspace_comp_results $split[1] $split[1].
                __devspace_debug "Completions are now: $__devspace_comp_results"
            end
        end
...
if type -q "devspace"
    ...
    complete --do-complete "devspace " > /dev/null 2>&1
end

# Remove any pre-existing completions for the program since we will be handling all of them.
complete -c devspace -e

# The call to __devspace_prepare_completions will setup __devspace_comp_results
